...
if [ -f "/usr/lib/enigma2/python/Plugins/Extensions/MeFullBackup/speed.txt" ] ; then
	ESTSPEED=`cat /usr/lib/enigma2/python/Plugins/Extensions/MeFullBackup/speed.txt`
else
	ESTSPEED="250"
fi
...
START=$(date +%s)
LOOKUP="/usr/lib/enigma2/python/Plugins/Extensions/MeFullBackup/lookuptable.txt"
TARGET="XX"
...
echo "*** THIS BACKUP IS CREATED WITH THE PLUGIN MeFullBackup ***" > $LOGFILE
echo "***** This plugin is brought to you by muzoss******" >> $LOGFILE
echo $LINE >> $LOGFILE
...
echo $SPEED > /usr/lib/enigma2/python/Plugins/Extensions/MeFullBackup/speed.txt
echo $LINE >> $LOGFILE
