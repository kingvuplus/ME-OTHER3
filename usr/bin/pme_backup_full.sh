...
export LANG=$1
export SHOW="python /usr/lib/enigma2/python/Plugins/Extensions/MeFullBackup/message.py $LANG"
export HARDDISK=1
echo -n $YELLOW
...
if [ -f /hdd/hdd-check ] ; then  
	...
	if  [ $MEDIA = "0" ] ; then
		...
		echo -n $WHITE
		/usr/bin/backupsuite.sh $MEDIA 
		echo "$HDD_MOUNT" > /tmp/MeFullBackup.log
		sync
	fi
fi
